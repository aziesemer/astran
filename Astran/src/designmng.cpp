...
void DesignMng::run(string filename) {
	...
	while(getline(file, str_tmp)){
		if((str_tmp[0] != '*') && !readCommand(str_tmp))
            throw AstranError("Could not execute line: " + line);
		line++;
	}
}
        ...
        if (words.size() != 0){
            switch (getCommandCode(words)){
                ...
                case CELLGEN_COMPACT:
                    autocell->compact(lpSolverFile, atoi(words[2].c_str()), atoi(words[3].c_str()), atoi(words[4].c_str()), atoi(words[5].c_str()), atoi(words[6].c_str()), atoi(words[7].c_str()));
                    break;
